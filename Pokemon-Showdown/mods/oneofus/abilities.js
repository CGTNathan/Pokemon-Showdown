...
exports.BattleAbilities = {
	...
	"steelheart": {
		...
		isNonstandard: true,
		onSourceModifyDamage: function (damage, source, target, move) {
			if (move.typeMod > 0 || move.typeMod === 0 || move.typeMod < 0) {
				this.debug('Filter neutralize');
				return this.chainModify(0.75);
			}
		}
	...
	"darkpresence": {
		...
		onModifyMove: function (move, pokemon) {
			if (move.type === 'Dark') {
			this.heal(pokemon.maxhp / 32);
			}
		},
		onTryHit: function (pokemon, target, move) {
			if (move.id === 'captivate' || move.id === 'taunt' || move.id === 'disable') {
				this.add('-immune', pokemon, '[msg]', '[from] Dark Presence');
				return null;
			}
		},
		onDamage: function (damage, target, source, effect) {
		...
		onImmunity: function (type, pokemon) {
			...
			if (type === 'frz') return false;
			if (type === 'attract') {
				this.add('-immune', pokemon, '[from] Dark Presence');
				return false;
			}
		},
		onFlinch: function () {
	...
	"harvesting": {
		...
		onResidual: function (pokemon) {
			if (this.isWeather(['sunnyday', 'desolateland']) || this.random(2) === 0) {
				if (pokemon.hp && !pokemon.item && this.getItem(pokemon.lastItem).isBerry) {
					pokemon.setItem(pokemon.lastItem);
					this.add('-item', pokemon, pokemon.getItem(), '[from] ability: Harvesting');
				}
			}
		},
		onTryHit: function (target, source, move) {
	...
	"objection": {
		...
		onHit: function (target, source, move) {
			if (target.hp && move.category !== 'Status' && !move.damage && !move.damageCallback && move.typeMod > 0 && target.useItem()) {
				this.boost({spe: 2});
				this.add('c|#Void Chrono|Objection!');
			}
		}
	},
	"leaguehopper": {
	...
	"safetyfirst": {
		...
		onStart: function (pokemon) {
			...
			for (var j in sideConditions) {
				if (pokemon.hp && pokemon.side.removeSideCondition(j)) {
					this.add('-sideend', pokemon.side, this.getEffect(j).name, '[from] ability: Safety First', '[of] ' + pokemon);
				}
			}
	...
	"simonitis": {
		...
		onStart: function (pokemon) {
			var target = pokemon.side.foe.active[pokemon.side.foe.active.length - 1 - pokemon.position];
			if (!target.hasAbility('simonitis')) {
				if (!target.volatiles['substitute']) {
					this.add('-ability', target, 'Simonitis', '[from] Simonitis');
					target.setAbility('simonitis');
				}
			}
		},
		onResidualOrder: 10,
		onResidualSubOrder: 1,
		onResidual: function (pokemon) {
			var infect = [0,1][this.random(2)];
			var target = pokemon.side.foe.active[pokemon.side.foe.active.length - 1 - pokemon.position];
			if (target && !target.hasAbility('simonitis')) {
				if (!target.volatiles['substitute']) {
					if (infect === 1) {
						var typeChange = ['Fire', 'Water', 'Electric', 'Grass', 'Ice', 'Psychic', 'Dragon', 'Dark', 'Fairy', 'Normal', 'Fighting', 'Poison',  'Ground', 'Flying', 'Bug', 'Rock', 'Ghost', 'Steel'][this.random(18)];
						this.add('-ability', target, 'Simonitis', '[from] Simonitis');
						target.setAbility('simonitis');
						if (target.hasType(typeChange)) {
							return;
						} else {
							this.add('-start', target, 'typechange', typeChange, '[from] Simonitis');
							target.setType(typeChange);				
						}
					}
				}
			}			
			var typeChange = ['Fire', 'Water', 'Electric', 'Grass', 'Ice', 'Psychic', 'Dragon', 'Dark', 'Fairy', 'Normal', 'Fighting', 'Poison',  'Ground', 'Flying', 'Bug', 'Rock', 'Ghost', 'Steel'][this.random(18)];
			if (!pokemon.hasType(typeChange)) {
				this.add('-start', pokemon, 'typechange', typeChange, '[from] Simonitis');
				pokemon.setType(typeChange);				
			}
			this.directDamage(pokemon.maxhp / 16, pokemon, pokemon);
		}
	},
	"evilintent": {
		id: "evilintent",
	...
	"freezebitch": {
		...
		onAnySetWeather: function (target, source, weather) {
			if (this.getWeather().id === 'hail' && !(weather.id in {hail:1, primordialsea:1, deltastream:1})) return false;
		},
		onEnd: function (pokemon) {
			if (this.weatherData.source !== pokemon) return;
			for (var i = 0; i < this.sides.length; i++) {
				for (var j = 0; j < this.sides[i].active.length; j++) {
					var target = this.sides[i].active[j];
					if (target === pokemon) continue;
					if (target && target.hp && target.hasAbility('freezebitch')) {
						this.weatherData.source = target;
						return;
					}
				}
			...
			var target = pokemon.side.foe.active[pokemon.side.foe.active.length - 1 - pokemon.position];
			if (!target.hasAbility('freezebitch')) {
				this.clearWeather();
			}
		}
	},
	"tacoitis": {
		...
		onStart: function (pokemon) {
			var target = pokemon.side.foe.active[pokemon.side.foe.active.length - 1 - pokemon.position];
			if (!target.hasAbility('tacoitis')) {
				if (!target.volatiles['substitute']) {
					this.add('-ability', target, 'Tacoitis', '[from] Tacoitis');
					target.setAbility('tacoitis');
				}
			}
		},
		onResidualOrder: 10,
	...
	"fuckyoufeelthisis": {
		id: "fuckyoufeelthisis",
		name: "Fuck you feel this is?",
		isNonstandard: true,
		onStart: function (pokemon) {
			this.add('-ability', pokemon, 'Fuck you feel this is?');
		},
		stopAttackEvents: true,
	},
	"grammarcheck": {
